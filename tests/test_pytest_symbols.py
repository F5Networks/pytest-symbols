...
def test_ini_line_empty(testdir):
    """Test that a symbols file specified in config loads symbols.
...
def test_ini_line_single(testdir, ext, data):
    """Test that a symbols file specified in config loads symbols.
...
@pytest.mark.parametrize('ext1, data1', [
    ('.json', _SYMBOLS_PATTERN_JSON.format(1)),
    ('.yaml', _SYMBOLS_PATTERN_YAML.format(1))
])
@pytest.mark.parametrize('ext2, data2', [
    ('.json', _SYMBOLS_PATTERN_JSON.format(2)),
    ('.yaml', _SYMBOLS_PATTERN_YAML.format(2))
])
def test_ini_line_multiple(testdir, ext1, data1, ext2, data2):
    """Test that symbols files specified in config loads symbols.

    Args:
        testdir: The pytest plugin testing utility.
        ext1: The file extension for the first symbols file.
        data1: The file contents for the first symbols file.
        ext2: The file extension for the second symbols file.
        data2: The file contents for the second symbols file.
    """
    data_file1 = testdir.makefile(ext1, data1)
    data_file2 = testdir.makefile(ext2, data2)
    testdir.makeini("""
        [pytest]
        symbols =
            {0}
            {1}
    """.format(data_file1, data_file2))
    testdir.makepyfile("""
        def test(symbols):
            assert symbols
            assert symbols.client1_ip == '1.1.1.1'
            assert symbols.server1_ip == '2.2.2.2'
            assert symbols.client2_ip == '1.1.1.1'
            assert symbols.server2_ip == '2.2.2.2'
    """)
    result = testdir.runpytest()
    result.assert_outcomes(passed=1)
...
def test_ini_and_cli(testdir, ext1, data1, ext2, data2):
    """Test that symbols files specified in config loads symbols.
    ...
    testdir.makepyfile("""
        def test(symbols):
            ...
            assert symbols.server2_ip == '2.2.2.2'
    """)
    result = testdir.runpytest('--symbols', data_file2)
    result.assert_outcomes(passed=1)
